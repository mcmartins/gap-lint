...
QUnit.test('Null', function (assert) {
  var text = null;
  assert.ok(window.GapLint.comply(null).length == 0, 'Testing text: ' + text + ' - should return 0 results.');
});
...
QUnit.test('If statement', function (assert) {
  var text = 'if then \n fi;';
  assert.ok(window.GapLint.comply(text).length == 0, 'Testing text: ' + text + ' - should return 0 results.');
});
...
QUnit.test('Chunk of code', function (assert) {
  var text = '# Literals\n1234\n.123\n7\n2147483647\n(1,2,3)(4,5,6)\n\n"String literals"\n\n"""\nMultiline\nstring\nliterals\n"""\n9123really\w\ eir\_\(identifier := 5;\n\n\nf := function(a,b,c)\n\n local x,y,z,g\n\nreturn x;\nend;';
  var results = window.GapLint.comply(text);
